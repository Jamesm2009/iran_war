...
DATA.filters        // Risk filter labels and impact descriptions
...
For each of the 15 factors ask:
...
|🇨🇴 Colombia |Transport diesel cost vs fertiliser cost — double compound                               |
...
|🇨🇦 Canada   |Western Canada logistics; domestic refining capacity                                     |
...
```javascript
{ date:"June X", assessment:5, factor:"What changed",
...
```javascript
meta: { assessment:5, day:102, date:"June 10, 2026" }
```
...
- [ ] `filterMods` deltas still sum to zero across each row
...
```javascript
filterMods: {
  f1: [-5, -3,  8,  0],  // IRGC escalation: takes from Sc1/Sc2, adds to Sc3
  f2: [-4, -2,  6,  0],
  ...
}
```

Update these whenever a factor’s structural weight on the scenario balance changes materially.
